...
data.TH1 = data.TH1[data.TH1$Date < as.POSIXct('2022-08-25'),]
...
xx = rbind(data.TH1[, c('ID', 'Treatment', 'site')],
           rand.TH58[, c('ID', 'Treatment', 'site')],
           rand.TH57[, c('ID', 'Treatment', 'site')],
           data.BR3[, c('ID', 'Treatment', 'site')])
...
write.csv(x = xx, file = 'ITT_population.csv',row.names = F,quote = F)
