...
stan_inputs$All$Trt_matrix = 
  cbind(stan_inputs$All$Trt_matrix,
        as.numeric(platcov_dat_list$All$Variant!='Delta' & 
                     platcov_dat_list$All$Trt=='Casirivimab\nimdevimab'))
...
```{r}
all_res = which(model_settings$dataset=='All')
all_res_main = which(model_settings$dataset=='All' & main_mods)
...
for(i in all_res){
  ...
  rownames(effect_ests_all[[which(i==all_res)]])=trt_list[[3]][-1]
}
par(las=1, mar=c(5,10,2,2))
...
# for(i in 1:length(all_res_mod)){
#   load(paste0('Rout/model_fits_',all_res_mod[i],'.RData'))
#   theta_trts_all = extract(out, pars='trt_effect')$trt_effect
#   writeLines('Effect of Regeneron overall:')
#   print(round(100*(exp(quantile(theta_trts_all, probs = c(0.025, 0.1, .5, .9, .975)))-1), 1))
# }
# writeLines('For the main model:')
# load(paste0('Rout/model_fits_',all_res_mod_main,'.RData'))
# theta_trts_all = extract(out, pars='trt_effect')$trt_effect
# writeLines('Effect of Regeneron overall:')
# print(round(100*(exp(quantile(theta_trts_all, probs = c(0.025, 0.1, .5, .9, .975)))-1), 1))
```
